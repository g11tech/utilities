...
## Motivation
A wholesome guide to familiarize, summarize and provide pointers to deep dive while staying in  **[ethereum](https://coinmarketcap.com/currencies/ethereum/)**'s context.

##### Note: Hash is just a commitment to the value being hashed, proof is checking the integrity of Hash w.r.t data.
...
Similarly *root* of a *[merkel tree](https://en.wikipedia.org/wiki/Merkle_tree)* is a **commitment** to the leaves and their indexes, or in short to the map of `indexes => values`, *[proof](https://medium.com/crypto-0-nite/merkle-proofs-explained-6dd429623dc5)* is the *merkel branch* and its *sibling hashes* which can be used to check the consistency all the way to the *merkel root*.

##### Note2: correspondance between a data map and a polynomial
...
For optimizations, and for one to one mapping between polynomial and data map, instead of just `index` as `x` in `f(x)`, `w^index` is used i.e. `f(w^index)=value` where `w` is the `d`th [root of unity](https://en.wikipedia.org/wiki/Root_of_unity) where `d` is the degree of the polynomial (the maximum index we are trying to represent), so that [FFT](https://vitalik.ca/general/2019/05/12/fft.html) can be deployed for efficient polynomial operations like multiplication and division which are of the `O(d)` in evaluation form and can be converted back into *coefficient form* in `O(d*log(d))`. So its still beneficial to keep `d` small.
![](https://i.imgur.com/AdtiDuL.png)


##### Note2.1: Ethereum's state is a map from `addresses => (version,balance,nonce,codeHash,storageRoot)`
...
This problem is compounded even further (because of network transmission of the witness data) in following scenarios:
...
One of the goals of ETH POS is to enable to commit ~1.5MB/s of data (think this throughput as the state change throughput and hence transaction throughput) into the chain. To achieve this, many parallel proposals will be done and verified at any given *~`12` second slot*, and hence multiple *shard chains* will exists. Cross polination across the *shards* will happen through again this committed *shard* data. 
But since, clients will

Katte commitment can be used to create [data validity and  availability scheme](https://hackmd.io/@vbuterin/sharding_proposal) without  clients accesing full data published across shards by the shard proposer.
...
* `2048` chunks of these `32768` samples (each containing `16` samples i.e. `512 byte` samples) are published by the *shard proposer* horizontally (`i`th chunk going to 'i'th vertical subnet along with their respective proof), plus globally publishing the `commitment`.
